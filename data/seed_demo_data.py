#!/usr/bin/env python3
"""Seeds a running instance of the MLOps Platform with realistic demo data — several models, multiple versions each at different lifecycle stages, a mix of successful/failed/retried/rolled-back deployments, and
(via the deployment worker's automatic seeding) monitoring snapshots for anything that reaches PRODUCTION.

Runs against a server that's already up:

    python scripts/seed_demo_data.py
    python scripts/seed_demo_data.py --base-url http://localhost:8000

Safe to re-run: model names are unique, so a second run skips models that already exist (logged, not treated as an error) rather than failing or duplicating data.

Reads its data from scripts/seed_data.json — edit that file to change what gets seeded without touching this script.
"""
...
def promote_to(client: httpx.Client, ap_headers: dict, version: dict, target_stage: str) -> dict:
    """Walks PROMOTION_CHAIN from the version's current stage up to (and including) target_stage, one promotion call per step. A no-op if the version is already at/past the target (idempotent re-runs) — this
    includes PRODUCTION/ARCHIVED, which sit outside PROMOTION_CHAIN entirely and are never something to promote *away* from here."""
